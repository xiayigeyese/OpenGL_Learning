...
vec3 F_Fresnel_Schlick_roughness(float cosTheta, vec3 F0, float roughness)
{
	return F0 + (max(vec3(1.0 - roughness), F0) - F0) * pow(1 - cosTheta, 5.0f);
}
...
void main()
{
	...
	vec3 ks = F_Fresnel_Schlick_roughness(clamp(dot(N, V), 0.0, 1.0), F0, roughness);
	vec3 kd = (1 - ks) * (1 - metallic);
